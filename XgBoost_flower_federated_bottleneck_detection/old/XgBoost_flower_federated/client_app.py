...
from XgBoost_flower_federated.task import replace_keys, SingletonDataLoader, load_datasets
...
def client_fn(context):
    ...
    num_local_rounds = cfg["local-epochs"]
    
    filenames = cfg["resources"]["filenames"]
    remove_labels = cfg["resources"]["remove_labels"].values()
    features = cfg["resources"]["features"].values()
    seed = cfg["resources"]["seed"]
    
    clients_data_loaders, client_test_loaders, _, total_classes = data_loader.get_data_loaders(
        remove_labels=remove_labels, features=features, filenames=filenames, seed=seed
    )

    params = replace_keys(cfg["params"])
